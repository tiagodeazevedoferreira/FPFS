...
function displayEstatisticas() {
    ...
    const ordenacao = document.getElementById('ordenacao-estatisticas')?.value || 'classificacao';
    sortConfigEstatisticas.mode = ordenacao; // Atualiza o modo de ordenação
    ...
    if (sortConfigEstatisticas.mode === 'gols') {
        sortedTeams = Object.entries(golsPorTime).sort((a, b) => {
            ...
            return golsB - golsA; // Ordem decrescente
        });
        ...
        sortedTeams = Object.entries(golsPorTime).sort((a, b) => {
            ...
            return posA - posB; // Ordem ascendente
        });
    }
        ...
        options: {
            ...
            scales: {
                y: {
                    ...
                    title: {
                        display: false,
                        text: sortConfigEstatisticas.mode === 'gols' ? 'Quantidade de Gols' : 'Quantidade de Gols (Ordenado por Classificação)',
            ...
            plugins: {
                ...
                title: {
                    display: true,
                    text: sortConfigEstatisticas.mode === 'gols' ? 'Estatísticas por Gols Feitos (Decrescente)' : 'Estatísticas por Classificação',
...
function showTab(tabId) {
    ...
    if (tabId === 'classification') displayClassification();
        ...
        displayEstatisticas();
        // Adicionar ouvinte para ordenação apenas na aba Estatísticas
        const ordenacaoSelect = document.getElementById('ordenacao-estatisticas');
        if (ordenacaoSelect && !ordenacaoSelect.dataset.listenerAdded) {
            ordenacaoSelect.addEventListener('change', () => {
                sortConfigEstatisticas.mode = ordenacaoSelect.value;
                displayEstatisticas();
            });
            ordenacaoSelect.dataset.listenerAdded = 'true';
        }
